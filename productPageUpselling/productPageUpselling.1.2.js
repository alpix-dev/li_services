...
apx_widgets.worker.productPageUpselling.config = {
    valueFrom: '.preco-produto .titulo',
...
apx_widgets.functions.productPageUpsellingGet = function (k, query_, apx_widgetsproductPageUpselling){
    $.get(query_[k], function(data){
        ...
        let price = hasVariant ? '' : result.find('.principal .acoes-produto').find(apx_widgets.worker.productPageUpselling.config.valueFrom).html().trim();
        ...
        if(hasVariant){                    
            ...
            result.find('.principal .acoes-produto').each(function(k2, item){
                let price_opt = $(this).find(apx_widgets.worker.productPageUpselling.config.valueFrom).html().trim();
